...
// Get all elements with the class name 'card-container-small'
var elements = document.querySelectorAll('[name="rfb"]');
console.log(elements);
elements.forEach(function(element) {
    var count = 0;
    var divs = element.getElementsByTagName('img');
    var isOverflowing = isTextOverflowingContainer(element);
	console.log(count);
    while (isOverflowing && count < maxTries) {
        divs = element.getElementsByTagName('img');
        scaleDown(divs);
        isOverflowing = isTextOverflowingContainer(element);
        count++;
    }
});
...
function scaleDown(divs) {
    fontSize2 -= 0.5; // 
    divs.style.fontSize = "".concat(fontSize2, "px");
